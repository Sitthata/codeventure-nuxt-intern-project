    ...
    <div class="flex flex-row justify-center flex-grow">
      <div class="prose mr-4 p-8 bg-white rounded-md min-w-[20ch] flex flex-col">
        ...
        <div class="space-y-1 mb-4 flex flex-col" v-for="(chapter, index) in course.chapters" :key="chapter.slug">
          <h4 class="font-semibold flex justify-between">{{ chapter.title }}
            <span class="text-emerald-500 text-sm font-semibold" v-if="percentageCompleted && user">{{
              percentageCompleted.chapters[index] }}%</span>
          </h4>
          <NuxtLink v-for="(lesson, index) in chapter.lessons" :key="lesson.slug"
            class="flex flex-row space-x-1 prose-sm font-normal py-1 px-4 -mx-4" :to="lesson.path" :class="{
        ...
        <div class="text-center" v-if="percentageCompleted && user">
          Course Completion: <span class="text-emerald-500 font-semibold">{{ percentageCompleted?.course }}%</span>
        </div>
      </div>
      <NuxtErrorBoundary>
        <NuxtPage />
